...
using ElFinderNet7.Models.Responses;
...
namespace ElFinderWeb31.Controllers
{
    ...
    public class FilesController : Controller
    {
        ...
        public async Task<IActionResult> GetFile(string path)
        {
            ...
            var fullPath = Program.MapStoragePath(path);
        ...
        private async Task<(IVolume Volume, long Quota)> SetupConnectorAsync(CancellationToken cancellationToken)
        {
            ...
            var volume = new Volume(_driver,
                Program.MapStoragePath($"./upload/{volumePath}"),
                Program.TempPath,
                $"/api/files/storage/upload/{volumePath}/",
            $"/api/files/thumb/",
                thumbnailDirectory: Program.MapStoragePath($"./thumb/{volumePath}"))
            {
                Name = "My volume",
                MaxUploadFiles = 20,
